...
if __name__ == "__main__":
    print("=== (a) Comparison of Gamma_Ohmic: numerical vs. closed form ===")
    for t in [0.5, 1.0, 2.0, 5.0, 10.0]:
        g_num = Gamma_Ohmic_numeric(t)
        g_closed = Gamma_Ohmic_closed(t)
        match = abs(g_num - g_closed) < 1e-5
        print(f"  t={t:5.1f}  Gamma_numeric={g_num:.8f}  Gamma_closed={g_closed:.8f}  match={match}")

    print("\n=== (b) Testing the 1/d saturation law at arbitrary time points (Ohmic) ===")
    ...
    for t_test in [1.0, 3.0, 5.0, 10.0]:
        ...
        print(f"    C(fitted)={popt[0]:.6f}   max|fit_residual|={np.max(np.abs(fit_resid)):.2e}")