...
def find_peak_trough_r(omega0, lam, eta):
    # Initial coarse scan over a wide interval, to justify that the subsequent
    # fine-grained windows genuinely contain the true first global minimum/maximum, not just a guessed local extremum
    ...
    assert len(minima_idx) > 0 and len(maxima_idx) > 0, "Coarse scan found no extremum"
...
if __name__ == "__main__":
    ...
    for pset in param_sets:
        ...
        print(f"  --> Stability of C: mean={Cvals[-3:].mean():.5f}  relative deviation={rel:.5f}%   [{status}, threshold=1e-4%]")
        assert rel < 1e-4, f"Violation of the paper's claim for {pset}: relative deviation {rel}% >= 1e-4%"