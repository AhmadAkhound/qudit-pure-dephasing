...
def C_exact(r, kmax=20000):
    ...
    for k in range(1, kmax):
        term = k * r**(k**2)
        total += term
        if term < mpf(10)**(-60):   # automatic cutoff once terms become negligible
            break
    return total

if __name__ == "__main__":
    # (a) and (b): convergence and monotonicity -- checked across a wide range of r
    print("=== (a),(b) Convergence and monotonicity of C(r) ===")
    ...
    for r in rs_monotonic:
        ...
        print(f"  r={r:6.3f}  C(r)={float(val):.6f}  monotonic_so_far={monotonic_ok}")
        prev = val

    # (c) Asymptotic behavior near r=1
    print("\n=== (c) Asymptotic behavior of C(r) ~ 1/(2(1-r)) as r->1 ===")
    rs_asymp = [0.9, 0.99, 0.999, 0.9999, 0.99999, 0.999999]
    print(f"{'r':>10} {'C(r) exact':>16} {'1/(2(1-r))':>16} {'ratio C(r)*(1-r)':>18}")
    for r in rs_asymp:
        val = C_exact(r)
        asym = mpf(1) / (2*(1-mpf(r)))
        ratio = val * (1 - mpf(r))
        print(f"{r:10.6f} {float(val):16.4f} {float(asym):16.4f} {float(ratio):18.6f}  (should -> 0.5)")