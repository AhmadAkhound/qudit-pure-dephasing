...
if __name__ == "__main__":
    print("=== مقایسه‌ی تابع همبستگی: شبه‌مد ایده‌آل در برابر J(w) واقعی (w=0..inf) ===")
    print(f"{'tau':>6} {'|C_pseudomode|':>16} {'|C_from_J|':>14} {'نسبت':>10}")
    for tau in [0.1, 0.5, 1.0, 2.0, 4.0]:
        c1 = C_pseudomode(tau)
        c2 = C_from_J_exact(tau)
        print(f"{tau:6.2f} {abs(c1):16.6f} {abs(c2):14.6f} {abs(c1)/abs(c2):10.5f}")
    print("\n>>> اگر همه‌ی نسبت‌ها نزدیک ۱ (مثلا در بازه‌ی 0.97-1.03) بودند، تناظر پارامتری")
    print(">>> تقریبا درست است و اختلاف کوچک ناشی از برش فیزیکی w=0 است، نه باگ.")