...
mp.dps = 2500  # very high precision, needed to resolve the smallest bounds tested (~1e-1900)
...
if __name__ == "__main__":
    # the same r values obtained from the paper's real physical parameter regimes 
    ...
    print(f"Total checks: {total}   Actual violations: {len(violations)}")
    for v in violations:
        print("  Violation:", v)
    print(f"\n>>> Final result: {'The bound holds in all cases (2500-digit precision)' if all_ok else 'A violation was found -- further investigation needed'}")