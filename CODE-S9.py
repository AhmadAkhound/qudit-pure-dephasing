...
def Gamma(t, omega0, lam, eta):
    ...
    if err > 1e-9 * max(1.0, abs(val)):
        raise RuntimeError(
            f"Quadrature error too large: value={val}, error={err}"
        )
    return val
...
def find_peak_trough_r(omega0, lam, eta):
    # Initial coarse scan, to guarantee the validity of the subsequent fine-grained windows
...
if __name__ == "__main__":
    ...
    for pset in param_sets:
        ...
        print(f"  Extended model A-C/d-C2/d^2: C2={C2_fit:.4e}±{C2_err:.2e} (expected ~0)")
